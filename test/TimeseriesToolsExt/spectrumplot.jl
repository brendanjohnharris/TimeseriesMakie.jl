@testitem "SpectrumPlot" setup = [ToolsSetup] begin
    ...
    f = Figure()
    ax = Axis(f[1, 1])
    TimeseriesMakie.spectrumplot!(
        ax, x, y, peaks = true, annotate = true,
        offset = (0, 3)
    )
    display(f)
end
...
@testitem "SpectrumPlot TimeseriesTools" setup = [ToolsSetup] begin
    ...
    ax = Axis(
        f[1, 1]; dim1_conversion = uc, xscale = log10, yscale = log10,
        xticks
    )
    TimeseriesMakie.spectrumplot!(
        s[10:end], peaks = false, annotate = false,
        nonnegative = true
    )

    TimeseriesMakie.spectrumplot!(
        x, peaks = false, annotate = false,
    ...
    X = ToolsArray(
        [colorednoise(t[1:10:end] * u"s") .* i * u"V" for i in 1:10],
        Var(1:10)
    ) |>
        stack
    S = spectrum(X)
